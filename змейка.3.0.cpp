// змейка.3.0.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
int main()
{
	...
	system("mode con cols=51 lines=31"); // размер консоли
	MoveWindow(GetConsoleWindow(), 50, 50, 1000, 1000, true); // установка стартовой позиции окна консоли 
	const int width = 50, height = 30; // игровое поле

	const int max_length = 50; 
	
	int array_X[max_length]; // массив с абсциссы звеньев "змейки"
	int array_Y[max_length]; // массив с ординаты звеньев "змейки"
	int length = 1; // длины "змейки"
	array_X[0] = width / 2; // старт  абсциссы "змейки"
	
	array_Y[0] = height / 2; // старт ординаты "змейки"
	
	int dx = 1, dy = 0; // начало движения
	int X_apple; 
	int Y_apple; 
	...
	char snake = '*'; // тело "змейки"
	char apple = 'o'; // яблоко
	char head = 1; // голова змейки
	
	COORD c; // координат
	HANDLE h = GetStdHandle(STD_OUTPUT_HANDLE); // поток вывода
	CONSOLE_CURSOR_INFO cci = { sizeof(cci), false }; // курсор
